...
function importDirectory() {
    ...
    async.series(scheduleHookDirectoryFunctions(archive), function (error, results) {
        archive.finalize();
    });
}

function scheduleHookDirectoryFunctions(archive) {
    var dirFunctions = [];
    for (var mappedExtension in _hooks) {
        var hookFn = _hooks[mappedExtension].directoryFunction;
        if (hookFn) {
            scheduleHook(hookFn, dirFunctions, archive);
        }
    }
    return dirFunctions;
}

function scheduleHook(hookFn, fnArray, archive) {
    fnArray.push(function (callback) {
        hookFn(_absolutePath, archive, function (error) {
            if (error) {
                callback(error);
            } else {
                callback();
