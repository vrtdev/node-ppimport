...
function importSingleFile(absolutePath, callback) {
    if (path.extname(absolutePath).toUpperCase() === '.XML') {
        fs.createReadStream(absolutePath).pipe(request.defaults({headers: {'Content-Type': 'text/xml'}}).put(polopolyUrl, function (error, response, body) {
        ...
        if (error) {
            ...
            if (stats.isDirectory()) {
                ...
                archive.on('error', function(err) {
                    var error = new ArchiveError('Archiving failed, see cause for more info', err);
                    callback(error);
                });

                archive.bulk([
                    { expand: true, cwd: absolutePath, src: ['**/*.xml'] }
                ]);
                archive.finalize();
                callback();
            } else {
...
module.exports = function importContentConst(options) {
    polopolyOptions = options || {polopolyUrl: 'http://localhost', username: 'admin', password: 'admin'};

    polopolyUrl = polopolyOptions.polopolyUrl + "?result=true&username=" + encodeURIComponent(polopolyOptions.username) + "&password=" + encodeURIComponent(polopolyOptions.password);
    console.log(polopolyUrl);
